...
def protected():
    ...
    if not auth_header:
        return jsonify(message="Token e necessario!"), 403
...
def listar_produtos():
    ...
    if description_part:
        produtos_filtrados = [
            ...
            if description_part in p.get("product_description", "").lower()
        ]
        return jsonify(produtos_filtrados)
    if preco_asc:
        ordem = sorted(products, key=lambda p: p["product_price"])
        return jsonify(ordem)
    elif preco_desc:
        ordem = sorted(products, key=lambda p: p["product_price"], reverse=True)
        return jsonify(ordem)
